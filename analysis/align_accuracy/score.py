...
def load_embeddings(directory, LANGS, key):
    sentence_embeddings = {}
    sentence_texts = {}
    for lang in LANGS:
        sentence_embeddings[lang] = {}
        sentence_texts[lang] = {}
        embeddings = np.loadtxt(f"{directory}/{key}/{lang}_sent_avg_pool.csv", delimiter=",", dtype=np.float32)
        with open(f"{directory}/{lang}.txt") as sentence_file:
...
def compute_accuracy(directory, LANGS, key="align"):
    sentence_embeddings, sentence_texts = load_embeddings(directory, LANGS, key=key)
...
if __name__ == "__main__":
    ...
    compute_accuracy(args.directory, langs, args.directory.split('/')[-1])
